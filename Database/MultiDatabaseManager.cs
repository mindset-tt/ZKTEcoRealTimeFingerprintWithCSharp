...
namespace ZKTecoRealTimeLog.Database
{
    ...
    public class MultiDatabaseManager : IDisposable
    {
        ...
        private async Task TryAddDatabaseAsync(string typeName, IndividualDatabaseConfig config)
        {
            ...
            try
            {
                ...
                if (provider != null && await provider.TestConnectionAsync())
                {
                    ...
                    Console.WriteLine($"  ✓ {provider.ProviderName}: Connected to {config.GetConnectionInfo()}");
                    Console.ResetColor();
                }
                else
                {
                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine($"  ✗ {typeName}: Connection failed (skipped)");
                    Console.ResetColor();
                    provider?.Dispose();
                }
            }
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"  ✗ {typeName}: Error - {ex.Message}");
                Console.ResetColor();
            }
        }
            ...
            string? deviceName)
        {
            ...
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine($"  Warning: Failed to insert to {provider.ProviderName}: {ex.Message}");
                Console.ResetColor();
            }
        }

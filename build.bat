...
dotnet publish ZKTecoRealTimeLog.csproj -c Release -r win-x86 --self-contained true -p:PublishSingleFile=true -o "%PUBLISH_DIR%\x86"
...
dotnet publish ZKTecoRealTimeLog.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PlatformTarget=x64 -o "%PUBLISH_DIR%\x64"
...
dotnet publish ZKTecoRealTimeLog.csproj -c Release -r win-arm64 --self-contained true -p:PublishSingleFile=true -p:PlatformTarget=ARM64 -o "%PUBLISH_DIR%\ARM64"
...
copy "%PROJECT_DIR%.env.example" "%OUTPUT_DIR%\ZKTecoAttendance-x86\" >nul
...
copy "%PROJECT_DIR%README.md" "%OUTPUT_DIR%\ZKTecoAttendance-x86\" >nul
...
if exist "%PUBLISH_DIR%\x64\ZKTecoRealTimeLog.exe" (
    ...
    copy "%PROJECT_DIR%.env.example" "%OUTPUT_DIR%\ZKTecoAttendance-x64\" >nul
    ...
    copy "%PROJECT_DIR%README.md" "%OUTPUT_DIR%\ZKTecoAttendance-x64\" >nul
)
...
if exist "%PUBLISH_DIR%\ARM64\ZKTecoRealTimeLog.exe" (
    ...
    copy "%PROJECT_DIR%.env.example" "%OUTPUT_DIR%\ZKTecoAttendance-ARM64\" >nul
    ...
    copy "%PROJECT_DIR%README.md" "%OUTPUT_DIR%\ZKTecoAttendance-ARM64\" >nul
)

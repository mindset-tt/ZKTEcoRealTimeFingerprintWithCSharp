...
dotnet publish ZKTecoRealTimeLog.csproj -c Release -r win-x86 --self-contained true -p:PublishSingleFile=true -o "%PUBLISH_DIR%"
...
wix build Package.wxs -d ProjectDir="%PROJECT_DIR%" -d PublishDir="%PUBLISH_DIR%" -o "%OUTPUT_DIR%\ZKTecoAttendance-Setup.msi"
